...
public class RomanToInteger {
    ...
    public static int romanToInt(String s) {
        ...
        while (i < stop) {
            int current = ROMAN_TO_INTEGER.get(chars[i]);
            int next = ROMAN_TO_INTEGER.get(chars[i + 1]);
            if (current < next) {
                sum += next - current;
                i += 2;
            } else {
        ...
        return i == chars.length ? sum : sum + ROMAN_TO_INTEGER.get(chars[chars.length - 1]);
    }
}
